		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					msg := []byte{0x81, 0xce, 0x00, 0x02, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00}
					binary.BigEndian.PutUint32(msg[len(msg)-4:], track.Ssrc)
					peerConnection.SendRTCP(msg)
					fmt.Println("Sending RTCP")
				}
			}
		}()
